...
3. 路由
    ...
    # 前端性能的优化 和 路由的懒加载
        ...
        6. 样式写到头部
        7. js写到底部
        8. cdn 连接 减少自己浏览器的压力 ，也能有缓存
        9. 图片的懒加载
        10. 路由的懒加载
            使用：
                10.1 ：不用在头部用 import Film from './views/Film.vue' 引入模块。
                10.2 ：在 配置路由对照表【就是一个 url 地址 对应 怎么样的视图组件】中写成：
                      {
                        path: 'films', // 就是 url 路径;
                        component: () => import(/* webpackChunkName: "文件名字" */ './views/Film.vue')  // 就是引进的组件
                      }
